  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        <VoiceGroup label="中文" voices={zhVoices} selected={selected} onSelect={onSelect} />
        <VoiceGroup label="English" voices={enVoices} selected={selected} onSelect={onSelect} />
      </div>
    </div>
  );
}
...
function VoiceGroup({
  ...
  return (
    <div>
      <div className="text-xs text-[var(--muted-foreground)] mb-1.5">{label}</div>
      <div className="grid grid-cols-2 gap-2">
        {voices.map((voice) => (
          ...
          >
            ...
            >
              {voice.gender === 'female' ? <User size={14} /> : <User size={14} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-medium truncate">{voice.name}</div>
              <div className="text-xs text-[var(--muted-foreground)] truncate">{voice.style}</div>
            </div>
          </button>
        ))}
      </div>
