  ...
  if (items.length === 0) {
    return (
      <div className="flex-1 p-4">
        <div className="text-sm text-[var(--muted-foreground)] text-center py-6">暂无历史记录</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between p-3 border-b border-[var(--card-border)]">
        <span className="text-xs font-medium text-[var(--muted-foreground)]">历史记录</span>
        <button
          onClick={onClear}
      ...
      <div className="flex-1 overflow-y-auto">
        {items.map((item) => (
          <div key={item.id} className="p-3 border-b border-[var(--card-border)] hover:bg-[var(--muted)]/10 group">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-xs text-[var(--muted-foreground)]">{item.voice || item.mode}</span>
                  <span className="text-xs text-[var(--muted)]">{new Date(item.timestamp).toLocaleTimeString()}</span>
                </div>
              </div>
            </div>
          </div>
