...
export default function StudioPage() {
  ...
  const handleModeChange = useCallback((newMode: TTSMode | 'asr') => {
    setMode(newMode);
    if (newMode !== 'asr') {
  ...
  return (
    <AppShell
      ...
      topBar={<TopBar mode={mode} voice={mode === 'preset' ? selectedVoice.name : undefined} />}
    >
      <div className="flex h-full">
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {mode === 'asr' ? (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {mode === 'preset' && (
                <VoiceSelector voices={PRESET_VOICES} selected={selectedVoice} onSelect={setSelectedVoice} />
              )}
              {mode === 'clone' && <VoiceSampleUpload file={voiceSample} onChange={setVoiceSample} />}
              <ContextControl mode={mode} value={context} onChange={setContext} />
              <GenerateButton
        ...
        <div className="w-80 border-l border-[var(--card-border)] flex flex-col overflow-hidden">
          <CurrentParams mode={mode} voice={selectedVoice.name} context={context} generating={generating} />
          <AudioPlayer audioUrl={audioUrl} />
          <HistoryPanel items={history} onPlay={handlePlayHistory} onClear={handleClearHistory} />
        </div>
      </div>
    </AppShell>
  );
